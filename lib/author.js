...
export async function getUserOrganizations(userId) {
  // Попробуем найти организации через проекты пользователя
  try {
...
export async function getOrganizationInfo(orgId) {
  // Попробуем получить информацию об организации через поиск проектов
  try {
    ...
    if (response.ok) {
      ...
      if (data.hits && data.hits.length > 0) {
        ...
        // Создаем объект организации на основе первого проекта
        const orgName = getOrganizationNameFromId(orgId);
        return {
          id: orgId,
          name: orgName,
          title: orgName,
          icon_url: firstProject.icon_url, // Используем иконку первого проекта
...
function getOrganizationNameFromId(orgId) {
  ...
  const knownOrgs = {
    'LjcZDkRW': 'CaffeineMC',
    // Можно добавить другие известные организации
  };
